    ...
    long a,ans;
     int c,g,h,s,r;
      ...
      {  
          ...
          if(a==0)
          {
              out.println((long)((long)M*(long)N));
          }
          else
          {
              if((long)((long)a*(long)M)%2==0)
              {
                  if(a%2==0)
                  {
                      if(M%2==0)
...
public static long func(long a)
{
 ...
 public static String toBinary(long x) {
        return String.format("%30s", Long.toBinaryString(x)).replaceAll(" ", "0");
    }
    static int primitive_root(int a,int b)
    {
        int j,i,s=1;
        boolean flag=true;
        for(j=2;j<b;j++)
